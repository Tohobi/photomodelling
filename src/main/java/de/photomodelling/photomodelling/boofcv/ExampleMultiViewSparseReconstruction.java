...
package de.photomodelling.photomodelling.boofcv;
...
public class ExampleMultiViewSparseReconstruction {
	...
	public List<Point3D_F64> runExample() {
...
//		example.compute("stone_sign.mp4", true);
		return example.visualizeSparseCloud();
	}
	...
	public void compute( String videoName, boolean sequential ) {
		...
		String path = "data/example/mvs/" + videoName;
		...
		// TODO: Put images to process into imageFiles variable

		if (imageDirectory.exists()) {
			imageFiles = UtilIO.listSmart(String.format("glob:%s/images/*.png", workDirectory), true, ( f ) -> true);
		} else {
			// TODO: Remove because not processing videos
		...
		// TODO: Need more images to have this working
		if (working == null)
			metricFromPairwise();
		if (scene == null)
	 ...
	 * scene.
	 */
	public List<Point3D_F64> visualizeSparseCloud() {
		checkTrue(scene.isHomogeneous());
		// TODO: Return cloudXyz and map to JSON response for Angular App to render
